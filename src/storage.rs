...
impl Account {
    fn increment_nonce(&mut self) -> Result<(), StorageError> {
        self.nonce = self.nonce.checked_add(1).ok_or(StorageError::NonceOverflow)?;
        Ok(())
    }

    fn increment_crit(&mut self, amount: CritAmount) -> Result<(), StorageError>{
        self.crit_balance = self.crit_balance.checked_add(amount).ok_or(StorageError::CritOverflow)?;
        Ok(())
    }
    fn decrement_crit(&mut self, amount: CritAmount) -> Result<(), StorageError> {
        self.crit_balance = self.crit_balance.checked_sub(amount).ok_or(StorageError::InsufficientCrit)?;
        Ok(())
    }

    fn unstake(&mut self, amount: StakeAmount) -> Result<(), StorageError> {
        let mut account_copy = self;
        account_copy.stake_balance = account_copy.stake_balance.checked_sub(amount).ok_or(StorageError::InsufficientStake)?;
